var Events = function(){
	...
	purchase_delete();
	add_more_fields();
	create_sale_after_post();
	create_expense_after_post();
    };
    ...
    var create_expense_after_post = function(){
	
	$('#create_expense_after_post').submit(
	    function(e){
		...
		$.post( url , data , 
			...
			setTimeout(
				...
				$('#some_message').removeClass('alert alert-success alert-dismissable');
				$('#some_message').html('');
				window.location.href = window.location.protocol + "//" + window.location.host + "/marblefactory/expense";
			    }
			, 6000);
		    }
		, "json");
	...
	$('#create_sale_after_post').submit(
		...
		$.post( url , data , 
			...
			setTimeout(
				...
				$('#some_message').removeClass('alert alert-success alert-dismissable');
				$('#some_message').html('');
				window.location.href = window.location.protocol + "//" + window.location.host + "/marblefactory/sale";
			    }
			, 6000);
		    }
		, "json");
